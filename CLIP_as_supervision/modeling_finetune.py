...
class iTPN(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 ...
                 norm_layer=nn.LayerNorm, patch_norm=False, use_checkpoint=False, num_classes=1000, use_mean_pooling=False,
                 init_std=0.02,
                 init_scale=0.01,
                 cls_token=False,
                 grad_ckpt=False,
...
@register_model
def itpn_base_rope_fusedLN_NaiveSwiGLU_subln_xavier_3324_patch16_256(pretrained=False, **kwargs):
    model = iTPN(
       cls_token=False,
       patch_size=16, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24, num_heads=8, bridge_mlp_ratio=3.,
       mlp_ratio=4 * 2 / 3, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
...
@register_model
def itpn_base_xattn_fusedLN_NaiveSwiGLU_subln_RoPE_3324_patch16_224(pretrained=False, **kwargs):
    model = iTPN(
        patch_size=16, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24, num_heads=8, bridge_mlp_ratio=3.,
        mlp_ratio=4 * 2 / 3, qkv_bias=True, norm_layer=partial(FusedLayerNorm, eps=1e-6),
        subln=True,
        xattn=True,
        naiveswiglu=True,
        rope=True,
        pt_hw_seq_len=14,  # attention here: eva use patch14 and 16 here -- 224/14
        intp_freq=True,
        **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
...
@register_model
def itpn_large_xattn_fusedLN_NaiveSwiGLU_subln_RoPE_2240_patch16_224(pretrained=False, **kwargs):
    model = iTPN(
        patch_size=16, embed_dim=768, mlp_depth1=2, mlp_depth2=2, depth=40, num_heads=12, bridge_mlp_ratio=3.,
        mlp_ratio=4 * 2 / 3, qkv_bias=True, norm_layer=partial(FusedLayerNorm, eps=1e-6),
        subln=True,
...
@register_model
def itpn_large_fusedLN_NaiveSwiGLU_subln_xavier_2240_patch16_224(pretrained=False, **kwargs):
    model = iTPN(
...
@register_model
def itpn_large_xattn_fusedLN_NaiveSwiGLU_subln_RoPE_2240_patch16_256(pretrained=False, **kwargs):
    model = iTPN(
        img_size=256,
        patch_size=16, embed_dim=768, mlp_depth1=2, mlp_depth2=2, depth=40, num_heads=12, bridge_mlp_ratio=3.,
        mlp_ratio=4 * 2 / 3, qkv_bias=True, norm_layer=partial(FusedLayerNorm, eps=1e-6),
        ...
        naiveswiglu=True,
        rope=True,
        pt_hw_seq_len=16,  # attention here: eva use patch14 and 16 here -- 224/14
        intp_freq=True,
        **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
