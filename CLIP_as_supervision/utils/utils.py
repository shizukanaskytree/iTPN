...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None):
    output_dir = Path(args.output_dir)
    print('start auto load model >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    # if loss_scaler is not None:
    # import pdb; pdb.set_trace()
    if getattr(args, 'load_init', None) is not None:
        print('start auto load model aaaaaaaaaaa >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    ...
    if not getattr(args, 'enable_deepspeed', False):
        print('start auto load model bbbbbbbbbbbbbbbbb >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
        # torch.amp
        ...
        print('start auto load model ccccccccccccccc >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
        # deepspeed, only support '--auto_resume'.
        if args.auto_resume:
            import os
            print('output_dir :', output_dir)
            os.system('ls /cache/output')
